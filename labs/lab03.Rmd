...
confidence_interval <- function(x) {
  ...
  t_score <- qt(p = .975, df = sample_size - 1)
...
confidence_interval(pdxTrees$DBH)
```
...
```{r}
histo <- function(data, x, fill, color){
   x <- enquo(x)
   fill <- enquo(fill)
  ggplot(data = data, mapping = aes(x = !!x, fill = !!fill)) +
    geom_histogram(border = "color")
}

# Test it
histo(pdxTrees, DBH, Collected_By, blue)
```
